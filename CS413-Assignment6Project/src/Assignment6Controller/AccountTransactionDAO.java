...
public class AccountTransactionDAO implements DAOInterface<BankAccountTransaction> {
    ...
    public List<BankAccountTransaction> getTransactionsByAccountId(int accountId) throws SQLException {
        ...
        while (result.next()) {
            ...
            transaction.setID(result.getInt("acct_id"));
            transaction.setCreateDate(result.getTimestamp("create_date"));
            transaction.setAmount(result.getDouble("amount"));
            transaction.setDescription(result.getString("summary"));
            transaction.setType(result.getString("tran_type"));
